	...
	function displayBanquetes() {
		
				if(idioma=='en'){
					document.getElementById('titulo_pagina').innerHTML="Banquet";
				}
				
				
				$("body").attr("return_page","banquetes.html");
				...
				if(mynamespace.db){
						mynamespace.db.readTransaction(
							function (t) {
								t.executeSql('SELECT id, nombre,nombre_en  FROM contenidos WHERE hoteles_id = '+hoteles_id+' AND tipo = "Banquete" ORDER BY nombre ASC', [], 
											function (tx, results) {
													var i;
